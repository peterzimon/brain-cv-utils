...
void CvUtils::update() {
	...
	// --- Button A solo tap: cycle modes ---
	// Detect release of A when B is not pressed and wasn't a long press
	static bool prev_a = false;
	if (prev_a && !button_a_pressed_ && !button_b_pressed_) {
		next_mode();
	}
	prev_a = button_a_pressed_;
...
void CvUtils::next_mode() {
	...
	// Brief LED flash to show new mode
	update_mode_leds();
